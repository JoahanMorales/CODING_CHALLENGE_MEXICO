    ...
    <div className="grid gap-8">
      <section className="grid gap-4 lg:grid-cols-[1.08fr_0.92fr]">
        <div className="rounded-3xl border border-sky-100 bg-white p-6 shadow-sm shadow-sky-100/70">
          <p className="font-mono text-[10px] font-black uppercase text-sky-700">Benchmark paper inmutable</p>
          <div className="mt-4 flex flex-wrap items-end justify-between gap-4">
            ...
            <Label>Benchmark paper</Label>
          </div>
          <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">
            <Metric label="Mercados" value={String(benchmark?.venueCount ?? "--")} tone="sky" />
            ...
            <Metric label="Pérdida evitada" value={`$${benchmark?.shadowLearning.avoidedLossUsd ?? "--"}`} tone="amber" />
          </div>
        </div>

        <div className="rounded-3xl border border-emerald-100 bg-emerald-50/55 p-6">
          <p className="font-mono text-[10px] font-black uppercase text-emerald-700">Validación TEST_ORDER firmada</p>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            <Metric label="Mercado" value={benchmark?.testOrderValidation.venue ?? "--"} tone="emerald" />
            <Metric label="Fondos movidos" value={benchmark?.testOrderValidation.fundsMoved ? "SÍ" : "NO"} tone="emerald" />
          </div>
          <div className="mt-4 rounded-xl border border-emerald-200 bg-white/70 px-4 py-3 text-xs font-black text-emerald-700">Sin ejecución con dinero real</div>
        </div>
      </section>
      ...
      <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="font-mono text-[10px] font-black uppercase text-sky-700">Salud del gateway en vivo</p>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-4">
          <Metric label="Mercados en vivo" value={summary ? `${summary.exchanges.filter((exchange) => exchange.status === "live" || exchange.status === "polling").length}/7` : "--"} tone="emerald" />
          <Metric label="Universo del scanner" value={summary ? String(summary.scannerUniverse.length) : "--"} tone="sky" />
          <Metric label="Señales en vivo" value={summary ? String(summary.metrics.opportunitiesDetected) : "--"} tone="zinc" />
          <Metric label="Latencia en vivo" value={summary ? `${summary.metrics.averageDetectionLatencyMs}ms` : "--"} tone="sky" />
        </div>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-6">
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            ...
            <Metric label="Pérdidas evitadas" value={String(benchmark?.shadowLearning.avoidedLosses ?? "--")} tone="emerald" />
            <Metric label="Monto evitado" value={`$${benchmark?.shadowLearning.avoidedLossUsd ?? "--"}`} tone="emerald" />
            <Metric label="Acierto del modelo" value={`${benchmark?.shadowLearning.hitRatePct ?? "--"}%`} tone="sky" />
          </div>
        </div>
      </section>
    </div>
