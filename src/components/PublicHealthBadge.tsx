  ...
  return (
    <div className="grid gap-3 rounded-2xl border border-sky-100 bg-white/85 p-4 shadow-sm shadow-sky-100/60">
      ...
      <p className="text-xs font-semibold leading-5 text-zinc-500">
        {offline ? "La experiencia editorial sigue disponible. Inicia Railway o el gateway local para ver datos en vivo." : `Scanner activo en ${summary?.scannerUniverse.length ?? 0} mercados. Última lectura ${summary?.time ? new Date(summary.time).toLocaleTimeString() : "--"}.`}
      </p>
    </div>
  );
}
