...
**Backtest what-if en vivo.** Debajo del ControlDeck, un panel toma una muestra de dislocaciones cross-venue **reales** (`npm run backtest:sample <tape>` → `public/data/backtest-tape.json`) y re-aplica tus gates actuales — min edge, estrés de fees, tolerancia de slippage, profundidad, frescura, venues activos — recalculando trades ejecutables y P&L neto estimado **cada vez que mueves un slider**. Aplica los mismos gates económicos del motor (no re-corre el comité ML/AET), así que es una **cota inferior honesta** del efecto de la parametrización sobre datos grabados reales. Ejemplo real (60,716 rondas): con el preset Conservador → 0 trades; con el default → 3 trades / +$5.36; con Agresivo → 5 trades.
